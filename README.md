...
`f.process_chrom()` performs three steps of the analysis in sequence:

 - `f.prepare_data()`
 - `f.estimate_disp()`
 - `f.lrt()`

The intermediates generated by each step are described in more detail in the
table below.
...
| `prepare_data()`    | `bias`          | `(n_rows, n_reps)`          | Bias vectors                                |
...
| `threshold_chrom()` | `sig_<f>_<s>`   | JSON                        | Significantly differential clusters         |
| `threshold_chrom()` | `insig_<f>_<s>` | JSON                        | Constitutive clusters                       |
